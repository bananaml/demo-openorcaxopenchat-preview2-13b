...
Build the model as a Docker image. You can change the `openorcaxopenchat-preview2-13b` part to anything.

```sh
docker build -t openorcaxopenchat-preview2-13b .
```

Run the Potassium server

```sh
docker run --publish 8000:8000 -it openorcaxopenchat-preview2-13b
```
